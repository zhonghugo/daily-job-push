...
> 方便给一份简历吗？直接粘贴文本，或给我一个文件路径（pdf/txt/md 都行）。
> 说明一下：「匹配理由」是拿简历和岗位要求对出来的，有简历才写得准。不方便传就跳过，理由会按你的画像方向写。

拿到后：存 `~/.workbuddy/daily-job-push/resume.md`（pdf 先转纯文本），并在 profile.json 写 `"resume_path"`。⚠️ 简历原文只留本地，不得写进表格、摘要或推送。用户跳过就什么都不写，不追问。
