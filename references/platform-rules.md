...
**授权前置，运行中不打断**——是否授权登录在**开跑确认**时一次性问清（画像里 `allow_login_fetch` 可持久化），之后整轮无需人工：

1. **换渠道找同岗**（始终首选，无论是否授权）：转载站 / 全职招聘网 / 企查查 / 公司官网常与 BOSS/智联同源，多数岗位这一步就能拿到完整 JD
2. **已授权**（开跑确认用户回「授权登录」，或画像 `allow_login_fetch=true`）→ 遇登录墙直接调用 `login-gated-web-extract` 技能：弹出**可见** Chrome（专用 profile），用户自己扫码/登录，零延迟后经 CDP 通道读取页面。抓到的 JD 写真实要点，来源平台仍记原平台，备注注明「经用户授权登录抓取」
   - 登录态存在专用浏览器 profile 里，**跨运行复用**——本轮多次遇到登录墙不重复弹窗；下次运行登录态仍有效则全自动，**失效才提示重新授权**（手动触发时在开跑确认里问，定时运行自动降级兜底）
   - ⚠️ 绝不截图二维码让用户扫（二维码 ~2 分钟失效，截图往返必然过期）——这是 login-gated-web-extract 的核心教训，照它的流程走
3. **未授权 / 未应答（含定时自动化）**：**不问、不打断**——换渠道 → 仍无则定位链接兜底 + 要点写规范文案 `未获取到JD正文（<平台>需登录），职责要求以原始页面为准`；摘要注明「N 条登录墙岗位未抓 JD，开跑确认时回『授权登录』可补抓」
...
非 `detail` 的逐条处理：能补明细页就补（按上面的降级链）；补不到就改成定位链接 + 备注；带 `utm_*`/`pgRef`/`d_sfrom` 等跟踪参数的，**清洗成干净 URL**（只删来路/追踪参数，**带岗位 id 的参数不能删**——腾讯官网的 `?postId=` 删了就只剩一个键）。
