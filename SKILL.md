...
⚠️ **登录墙处置（BOSS直聘/智联等明细页需登录，WebFetch 只会拿到安全验证/登录页）**——按顺序走，不许跳步：

1. **先换渠道找同岗**（转载站/全职招聘网/企查查/官网），多数情况这一步就解决
2. 换渠道无果且登录墙岗位**累计 ≥2 条** → **问用户是否授权登录**（一次问清本批共几条，别逐条问）：「本批有 N 条岗位的 JD 在 BOSS/智联，需登录才能看。要我打开浏览器窗口、你扫码授权后我来抓吗？不授权的话这几条只留定位链接。」
   - **用户同意** → 走 `login-gated-web-extract` 技能：弹出可见 Chrome，用户自己扫码登录，随后经浏览器通道抓 JD 正文；登录态存在专用 profile 里**跨运行复用**，之后不再重复问，直到 cookie 失效再问一次
   - **用户拒绝 / 未应答（含定时自动化）** → 按定位链接兜底 + 要点写规范文案，摘要注明「N 条因未授权登录仅定位链接」
3. 只有 1 条且换渠道无果 → 不为单条打断用户，直接定位链接兜底，摘要里说明可下次授权补抓
...
- `岗位要点`：**必填，字段永不为空**。AI 提炼的 JD 要点（职责 + 硬性要求，≤60 字），只写页面上真实存在的内容。JD 正文拿不到（BOSS/智联需登录）→ 先回第 4 步补搜/换渠道，或经**用户授权登录**后用浏览器通道抓（见第 4 步登录墙处置）；都不可用 → 写规范文案 `未获取到JD正文（<平台>需登录），职责要求以原始页面为准`。**岗位不因此降级**（链接是明细页就仍是明细岗位）；只有**链接也拿不到**时才降级为线索。**不许留空，也不许脑补**
