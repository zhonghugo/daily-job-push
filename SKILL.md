...
3. 两轮后仍无 → 作「线索」入库（链接留空），**备注必须写**已试过的搜索词与找不到的原因；摘要单列「线索区」提示用户手动搜
...
- `岗位要点`：**必填**。AI 提炼的 JD 要点（职责 + 硬性要求，≤60 字），只写页面上真实存在的内容。JD 正文拿不到 → 回第 4 步补搜一轮；仍拿不到 → 降为线索（要点写"未获取到 JD，仅标题线索"）——**不许留空，也不许脑补**
