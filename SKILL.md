...
| `<out>.merges.json` | 归并计划（含 `键冲突`/`同名待确认` 标注），给人看 |
...
脚本会打四类标，**摘要必须原样带出，不许自己消化掉**：

| 标记 | 含义 | 怎么办 |
|---|---|---|
| `⚠️ 键冲突` | 一条候选同时命中**多条**已有记录 | 归并只改得动一条，其余靠人工合并 |
| `⚠️ 同链接重复`（存量体检） | 同一条 URL 挂在多行上、**岗位名一致** | 多半是重复行（历次重跑把公司名写飘了），人工合并 |
| `❗ 同链接但岗位名不同`（存量体检） | 同一条 URL 下**岗位名不一致** | 可能有行挂错了链接，**先核对再动**，别直接合 |
| `❓ 同名待确认` / `❓ 同名待查` | 公司+岗位名同、链接不同 | 可能同名不同岗（退化键会去括号内容）。候选那种已按同岗归并、两个链接都留在「同岗来源」里，误合由人拆；存量那种纯提醒 |

**存量体检（`── 存量体检（只读，不改记录）──`）是本版新增的只读巡检**：老脚本没脱 markdown 链接壳，链接去重对存量记录从未生效，表里因此攒下了重复行；归并只在候选碰到这些键时才改得动一条，候选碰不到就永远没人知道，所以每轮装载后全量扫一遍报出来。它**不改任何记录、不产生写回体**，纯粹是给人一张清理清单——清理是人工动作，别自动删行。
...
——— 以下只在本轮确有发现时出现 ———
⚠️ 键冲突：岗位C @公司C 同时命中 2 条已有记录——归并只改得动一条，其余需人工合并
❗/⚠️ 存量体检：7 组同链接、1 组同名待查（清单见下，只读巡检，不改记录）
...
**四类标记全部原样带出**（第 6 步输出的岗位名与记录 ID 直接抄，别复述、别省略），按上面第 6 步那张表处置：

- `⚠️ 键冲突` / `❓ 同名待确认` 是**本轮归并**（写库动作）碰上的，必须报——否则用户不知道有行的渠道数没写全
...
**改了 `scripts/` 里的脚本 → 先跑回归**：`bash scripts/test_dedup_keys.sh`（40 条断言，覆盖飞书/local 两模式、归并、四类标记、存量体检、同平台不涨渠道数）。不跑就等于拿用户的表当试验场。
