#!/bin/bash
# daily-job-push / dedup_keys.py 回归测试（40 条断言，用例 A–I）
...
chk "同名不同链接仍归并（不丢岗位）" "$(echo "$out"|grep -c '^  归并:')" "1"
chk "标注「同名待确认」" "$(echo "$out"|grep -c '同名待确认')" "1"
chk "两个链接都留在同岗来源里" "$(python3 -c "
import json;s=json.load(open('e.updates.json'))['update_records']['recTY1']['同岗来源']
print('zhipin' in s and 'liepin' in s)")" "True"
...
chk "带新链接的跨平台候选标同名待确认" "$(echo "$out"|grep -c '❓同名待确认')" "1"
